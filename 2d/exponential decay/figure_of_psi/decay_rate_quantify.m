...
ind_phi=num/2+N/(2*sizeH);
position=idx_global+shift(ind_phi); % position index of the domain
...
grad_psi=gradient(psi,hg,hg);

t_grid=1:N; [tt,ss]=meshgrid(t_grid*hg,t_grid*hg);
coef_a=kappa([tt(:),ss(:)]);
coef_a=reshape(coef_a,N,N);
norm_all=coef_a'.*grad_psi.^2*hg^2;
% norm_all=grad_psi.^2*hg^2;
norm_sum=sum(sum(norm_all));
...
position2=ind_tot;
norm_all2=norm_all(:);
norm_loc2=sum(norm_all2(position2));
