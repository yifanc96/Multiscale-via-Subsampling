...
u=A\f;
tic
uh=pde2d_subsample_solver_loc(A,f,N,sizeH,sizeh,1);
toc
...
figure;
mesh(xx,yy,reshape(u,N,N))
figure;
contourf(xx,yy,reshape(uh,N,N))
figure;
mesh(xx,yy,reshape(uh,N,N))
...
function [y]=force_f(x)
    ...
    tmp_cos=cos((1:k)'*x(:,1)');
    tmp_sin=sin((1:k)'*x(:,2)');
    y=0.2*sin(W1'*tmp_cos+W2'*tmp_sin); %row vector
end